<!-- StoryFields – the reusable content form for one story (Tour → Location → Story).
     Extracted from StoryEditor so it can be embedded in two places:
       • the standalone Story editor (its own screen, for locations with 2+ stories);
       • inline inside the Location editor, for the common 0–1-story case, so a
...
<template>
  ...
  <div>
    ...
    <span class="field-label" id="st-related-label">Related locations <span class="hint">shown as “Nearby stories” at the foot of this card</span></span>
...
const props = defineProps({
  ...
  // component mutates it in place (v-model) so the parent's save() sees the edits.
...
// in-body section toggles, seeded from whether the fields are already populated
...
// ── related locations (other stops) ──
const others = computed(() => store.locations.filter((l) => l.recordId !== form.locationId))
...
// Clear the fields behind any toggle the admin has turned off, so hidden content
...
// The set of image/audio URLs the story actually keeps – the parent uses this
// with sessionUploads to delete orphaned uploads after a save.
const inUseUrls = () => [form.heroImageUrl, form.historicImageUrl, form.sliderAfterUrl, form.portraitUrl, form.audioUrl].filter(Boolean)

defineExpose({ normalize, sessionUploads, inUseUrls })
</script>
