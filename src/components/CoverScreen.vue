...
<template>
  <div style="position: absolute; inset: 0; padding: 64px 26px 30px; display: flex; flex-direction: column;">
    ...
    <div style="font-size: 13px; font-weight: 600; letter-spacing: 1px; color: #9B6DFF; text-transform: uppercase;">{{ cityName }}</div>
    <h1 style="font-family: 'Bricolage Grotesque'; font-weight: 700; font-size: 58px; line-height: 0.92; letter-spacing: -2px; margin: 12px 0 0; white-space: pre-line;">{{ headline }}</h1>
    <p style="font-family: 'Newsreader'; font-size: 18px; line-height: 1.5; color: #D9CFE0; margin: 20px 0 0; max-width: 300px;">{{ intro }}</p>

    <div style="display: flex; gap: 18px; margin: 22px 0 26px; font-size: 12.5px; color: #A99BB8; font-weight: 600;">
    ...
    <div style="font-size: 12px; font-weight: 700; letter-spacing: 1px; color: #A99BB8; text-transform: uppercase; margin-bottom: 12px;">Choose how to explore</div>

    <button @click="$emit('guided')" :style="modeBtn" style="margin-bottom: 12px;">
      <span style="flex-shrink: 0; width: 46px; height: 46px; border-radius: 13px; background: linear-gradient(135deg, #9B6DFF, #3D9BFF); display: flex; align-items: center; justify-content: center;">
      ...
      <span style="flex: 1;">
        <span style="display: block; font-family: 'Bricolage Grotesque'; font-weight: 600; font-size: 18px;">Guided Tour</span>
        <span style="display: block; font-size: 13px; color: #A99BB8; margin-top: 2px;">Follow a set walking route, stop by stop</span>
      </span>
      <svg width="9" height="15" viewBox="0 0 9 15" fill="none"><path d="M1 1 L7.5 7.5 L1 14" stroke="#A99BB8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>

    <button @click="$emit('discovery')" :style="modeBtn">
      <span style="flex-shrink: 0; width: 46px; height: 46px; border-radius: 13px; background: linear-gradient(135deg, #FF4D5E, #FF8C42); display: flex; align-items: center; justify-content: center;">
      ...
      <span style="flex: 1;">
        <span style="display: block; font-family: 'Bricolage Grotesque'; font-weight: 600; font-size: 18px;">Discovery Mode</span>
        <span style="display: block; font-size: 13px; color: #A99BB8; margin-top: 2px;">Wander freely — we’ll alert you near a site</span>
      </span>
      <svg width="9" height="15" viewBox="0 0 9 15" fill="none"><path d="M1 1 L7.5 7.5 L1 14" stroke="#A99BB8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>
